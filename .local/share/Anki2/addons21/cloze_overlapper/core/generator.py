...
class ClozeGenerator:
    ...
    def generate(self, items, original=None, keys=None):
        ...
        for idx in range(self.start, self.total + 1):
            snippets = ["..."] * length
            start_c = self.get_cloze_start(idx)
            start_b = self.get_before_start(idx, start_c)
            end_a = self.get_after_end(idx)

            if start_b is not None:
                snippets[start_b:start_c] = self.remove_hints(items[start_b:start_c])
            if end_a is not None:
                snippets[idx:end_a] = self.remove_hints(items[idx:end_a])
            snippets[start_c:idx] = self.format_cloze(
                items[start_c:idx], idx - self.start + 1
            )

            field = self.format_snippets(snippets, original, keys)
            fields.append(field)
        nr = len(fields)
        if self.maxfields > self.total:  # delete contents of unused fields
            fields = fields + [""] * (self.maxfields - len(fields))
        fullsnippet = self.format_cloze(items, self.maxfields + 1)
        full = self.format_snippets(fullsnippet, original, keys)
        return fields, full, nr

    def format_cloze(self, items, nr):
    ...
    def format_snippets(self, snippets, original, keys):
    ...
    def get_before_start(self, idx, start_c):
